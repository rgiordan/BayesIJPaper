...
base_dir <- system2("git", args=c("rev-parse", "--show-toplevel"), stdout=TRUE)
output_dir <- file.path(base_dir, "src/singular_simulations/output")


ShouldRerun <- function(filename, force) {
  if (force) {
    return(TRUE)
  }
...
# This appears to be a good one that does everything we want!
...
desc <- sprintf("redim%d_obsperre%d_seed%d_TEST", re_dim, obs_per_re, seed_val)
...
base_filename <- file.path(output_dir, sprintf("super_simple_simulation_base_results_%s.Rdata", desc))
if (ShouldRerun(base_filename, force=FALSE)) {
  mcmc_time <- Sys.time()
  ...
  num_blocks <- 100
  num_draws <- 100
  ij_se <- GetBlockBootstrapCovarianceDraws(
    lp_draws, par_draws, num_blocks=num_blocks, num_draws=num_draws)
  bayes_se <- GetBlockBootstrapCovarianceDraws(
  ...
  save(df_base, mcmc_time, rstanarm_result, par_draws, ij_se, bayes_se,
       file=base_filename)
...
sim_filename <- file.path(output_dir, sprintf("super_simple_simulation_sim_results_%s.Rdata", desc))
pars <- c("x", "sigma", "Sigma[z:(Intercept),(Intercept)]")
if (ShouldRerun(sim_filename, force=FALSE)) {
  ...
  for (sim in 1:num_sims) {
    ...
    rstanarm_result_sim <- rstanarm::stan_glmer(
      model_formula, df_sim, family=gaussian(), iter=5000)
