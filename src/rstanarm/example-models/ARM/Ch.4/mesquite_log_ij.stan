...
transformed parameters {
  ...
  vector[N] lp_;
  for (n in 1:N) {
    lp_[n] =   normal_lpdf(log_weight[n] |
      beta[1] + beta[2] * log_diam1[n] + beta[3] * log_diam2[n]
      + beta[4] * log_canopy_height[n] + beta[5] * log_total_height[n]
      + beta[6] * log_density[n] + beta[7] * group[n],
      sigma);
